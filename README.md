...
Goroutine is a lightweight thread managed by the Go runtime. `go f(x,y,z)` runs a new  go routine with `f(x,y,z)` 
...
func change(a *Ab) {
    *v = 10
//  ^ used to change the base value of the variable
}

func main(){
    a := Ab(2)
    go change()
//       ^ used ot send the pointer to the variable
